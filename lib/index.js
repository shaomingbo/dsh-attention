...
export function createAttentionHost(options = {}) {
  ...
  function markVisibility(nextVisibility) {
    visibility = nextVisibility === 'visible' ? 'visible' : 'hidden'
    heartbeatAt = now()
    return { visibility, heartbeatAt }
  }

  function present() {
    return clientLooksPresent(visibility, heartbeatAt, now())
  }
  ...
  async function dispatch(event, source) {
    ...
    if (source === 'backup' && present()) {
      return { accepted: false, native: false, reason: 'client-visible' }
    }
    const key = event.key ?? attentionKey(event.kind, event.sessionId)
    if (!shouldAccept(seenAt, key, now())) {
      return { accepted: false, native: false, reason: 'deduped' }
    }
    ...
    try {
      ...
      if (typeof sessionId === 'string' && sessionId.length > 0) {
        ...
        await dispatch({
          ...
          key: attentionKey('approval', sessionId),
        }, 'backup')
      }
    } catch (error) {
  ...
  async function onAgentStatus({ agent, status }) {
    try {
      ...
      await dispatch({
        ...
        key: attentionKey('completed', sessionId),
      }, 'backup')
    } catch (error) {
