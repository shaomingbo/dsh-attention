...
export function createAttentionHost(options = {}) {
  ...
  /** Per-session completion ordinal: one unique identity per running→idle edge. */
  const completionSeq = new Map()
  /** Per-session approval ordinal: one unique identity per actual ask. */
  const approvalSeq = new Map()
  ...
  /**
   * The host is the sole identity minter for completions and approvals: every
   * producer (backup listener, any tab) reporting the same edge resolves to
   * the same canonical key, while a genuinely new edge gets a fresh ordinal.
   */
  function canonicalKey(kind, sessionId, requested) {
    if (kind === 'completed') {
      return `completed:${sessionId}:${completionSeq.get(sessionId) || 1}`
    }
    if (kind === 'approval') {
      return `approval:${sessionId}:${approvalSeq.get(sessionId) || 1}`
    }
    return requested
  }
  ...
  async function dispatch(event, source) {
    ...
    const key = canonicalKey(event.kind, event.sessionId, event.key)
  ...
  async function onApprovalRequest(req, next) {
    try {
      ...
      if (typeof sessionId === 'string' && sessionId.length > 0) {
        ...
        // Every waterfall entry is one distinct live ask: bump the ordinal
        // before dispatch so `callId`-less asks never share an identity.
        const seq = (approvalSeq.get(sessionId) ?? 0) + 1
        approvalSeq.set(sessionId, seq)
        await dispatch({
          kind: 'approval',
          sessionId,
          title: 'DeepSeek Harness',
          body: `${localizeKind('approval', 'en')}: ${toolName}`,
          key: `approval:${sessionId}:${seq}`,
        }, 'backup')
      }
    } catch (error) {
