...
const API_ENDPOINTS = ['prefs.get', 'prefs.set', 'visibility', 'notify']
...
export function createAttentionHost(options = {}) {
  ...
  /** Occupied completion edge per session: first reporter mints, others collapse. */
  ...
  function claimCompletion(sessionId) {
    const existing = pendingCompletion.get(sessionId)
    if (existing !== undefined) return `completed:${sessionId}:${existing}`
    const key = advanceKey('completed', sessionId)
    pendingCompletion.set(sessionId, kindSeq.get(`completed:${sessionId}`))
    return key
  }
  ...
  async function handle(endpoint, payload) {
    ...
    if (endpoint === 'notify') {
      ...
      if (phase === 'close') {
        ...
        if (kind === 'completed') pendingCompletion.delete(sessionId)
        return success({
          accepted: false, native: false, reason: 'closed',
          banner: false, sound: false, fallback: false, nativeAvailable,
        })
      }
      // Completions occupy one edge at a time: first reporter (tab or backup)
      // mints, everyone else collapses. Approvals collapse onto the waterfall
      // ordinal. Questions / plan-reviews occupy a live slot until close.
      let dispatchKey = key
      if (kind === 'completed') {
        dispatchKey = claimCompletion(sessionId)
  ...
  async function onAgentStatus({ agent, status }) {
    try {
      ...
      if (status === 'running' && previous === 'idle') {
        pendingCompletion.delete(sessionId)
        return
      }
...
export function apply(ctx) {
  ...
  registerLoopbackRpc(ctx, async (endpoint, payload) => {
    try {
