...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function AttentionRoot(props) {
       ...
       * natively, or another producer already delivered this exact event (the
       * host mints one identity per edge, so `deduped` proves a banner exists).
       * `soundOwned` — this call is what produced the notification; a deduped
       * echo must stay silent so concurrent tabs cannot double-chime.
       */
      const verdict = (result) => ({
        bannerHandled: result === null
          ? false
          : result.native === true || result.reason === 'deduped',
      ...
      React.useEffect(() => {
        ...
        // Waiting events share a 30 s bucket so concurrent tabs collapse to
        // one banner; approvals are additionally re-identified by the host's
        // per-ask ordinal, so a later ask always gets a fresh identity.
