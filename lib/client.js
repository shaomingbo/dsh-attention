...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function AttentionRoot(props) {
      ...
      /**
       * Interpret a host verdict. `bannerHandled` — the host showed the banner
       * natively, or another producer already delivered this exact event and a
       * banner truly went out (`banner` on the dedupe record). `soundOwned` —
       * this call is what produced the notification; a deduped echo must stay
       * silent so concurrent tabs cannot double-chime.
       */
      const verdict = (result) => ({
        bannerHandled: result === null
          ? false
          : result.native === true
            || (result.reason === 'deduped' && result.banner === true),
        soundOwned: result === null || result.accepted !== false,
      })
      ...
      const fireCompleted = React.useCallback((sessionId, displayTitle) => {
        ...
        void (async () => {
          const outcome = verdict(await notifyHost('completed', sessionId, displayTitle, key))
          if (state.prefs.sound && outcome.soundOwned) playTone('completed')
          if (!outcome.bannerHandled && state.prefs.browserNotification) {
      ...
      React.useEffect(() => {
        ...
        // Stable while THIS wait lives (so a long wait does not look like a
        // new one every 30 s), fresh per new wait (so the next approval in
        // the same session never collapses onto the previous one). The host
        // maps it onto its own per-ask ordinal either way.
        const key = `${attention.key}:${previousKey === null ? Date.now() : 'wait'}`
        void (async () => {
          const outcome = verdict(await notifyHost(attention.kind, attention.sessionId, attention.displayTitle, key))
          if (state.prefs.sound && outcome.soundOwned) playTone(attention.kind)
          if (!outcome.bannerHandled && state.prefs.browserNotification) {
