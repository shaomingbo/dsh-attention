...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    async function rpc(connection, endpoint, payload = {}) {
      const result = await connection.rpc.call(API_CHANNEL, `attention/${endpoint}`, payload)
    ...
    function showBrowserNotification(attention, locale, workspace) {
      ...
      notification.onclick = () => {
        window.focus()
        // 0.1.7 session navigation is the public uiWorkspace face.
        workspace?.openSession?.(attention.sessionId)
        notification.close()
      }
    }
    ...
    function selectRows(state) {
      ...
      for (const id of state.ids ?? []) {
        ...
        rows.push({
          ...
          // 0.1.7 rows carry interaction/completion facts through
          // useSessionStatus; `completed` below reads that snapshot.
      ...
      // 0.1.7 list rows no longer name a `current` entry; the foreground
      // session is the one the main view retains (`retainedBy.mainView`).
      let current
      for (const row of rows) {
        const session = state.byId?.[row.id]
    ...
    function AttentionRoot(props) {
      ...
      // 0.1.7 moves interaction/completion facts to the session-status hook.
      const statuses = typeof props.useSessionStatus === 'function' ? props.useSessionStatus(s => s) : undefined
      if (statuses instanceof Map) {
        for (const row of snapshot.rows) {
          const status = statuses.get(row.id)
          if (status === undefined) continue
          row.pendingInteraction = status.pendingInteraction
          row.completed = status.completionUnread === true
        }
      }
      const lastKey = React.useRef(null)
      const lastWait = React.useRef(null)
      ...
      /** Deliver one attention event; resolves to the host verdict (null on transport failure). */
      const notifyHost = React.useCallback(async (kind, sessionId, displayTitle, key, extra = {}) => {
        try {
      ...
      /**
       * Host grants: `sound` — this tab plays the chime; `fallback` — this tab
       * shows the browser notification. Concurrent / recovering tabs receive
       * neither grant, so they stay silent.
       */
      const applyOutcome = React.useCallback((result, kind, payload) => {
        if (result === null) {
          if (state.prefs.sound) playTone(kind)
          if (state.prefs.browserNotification) showBrowserNotification(payload, localeName, props.workspace)
          return
        }
        if (result.sound === true && state.prefs.sound) playTone(kind)
        if (result.fallback === true && state.prefs.browserNotification) {
          showBrowserNotification(payload, localeName, props.workspace)
        }
      }, [localeName, props.workspace, state.prefs])
      ...
      const fireCompleted = React.useCallback((sessionId, displayTitle) => {
        ...
        // The host rewrites completion keys to its per-session edge ordinal,
        // so every producer of the same edge lands on one identity. The client
        // key only needs to be unique per call for API validation.
        const key = `completed:${sessionId}:${Date.now()}:${Math.random().toString(36).slice(2, 8)}`
        ...
        void (async () => {
          applyOutcome(await notifyHost('completed', sessionId, displayTitle, key), 'completed', {
            kind: 'completed', sessionId, displayTitle, key,
          })
        })()
      }, [applyOutcome, flashCompletedTitle, localeName, notifyHost, state.loaded, state.prefs])
      ...
      // Running edges are tracked from the very first snapshot — before
      // preferences load — so a completion during startup is never lost: it is
      // stashed and flushed once `loaded` flips true (host defers to the
      // heartbeat, so without this flush that edge would notify nowhere).
      const stashedCompletions = React.useRef([])
      React.useEffect(() => {
        const pending = stashedCompletions.current
        for (const row of snapshot.rows) {
          const previous = prevRunning.current.get(row.id)
          if (previous === true && row.running === false) {
            if (state.loaded) fireCompleted(row.id, row.displayTitle)
            else if (!pending.some((entry) => entry.id === row.id)) pending.push(row)
          } else if (state.loaded && previous === false && row.running === true) {
            void notifyHost('completed', row.id, row.displayTitle, `completed:${row.id}:close`, { phase: 'close' })
          }
          prevRunning.current.set(row.id, row.running === true)
        }
      }, [fireCompleted, notifyHost, snapshot, state.loaded])

      React.useEffect(() => {
        if (!state.loaded || stashedCompletions.current.length === 0) return
        const pending = stashedCompletions.current
        stashedCompletions.current = []
        const rowsById = new Map(snapshot.rows.map((row) => [row.id, row]))
        for (const row of pending) {
          if (rowsById.get(row.id)?.running === false) fireCompleted(row.id, row.displayTitle)
        }
      }, [fireCompleted, snapshot, state.loaded])

      React.useEffect(() => {
        if (!state.loaded) return
        const wait = attention !== null && holdsTabAttention(attention.kind) ? attention : null
        const previous = lastWait.current
        if (previous !== null && (wait === null || wait.sessionId !== previous.sessionId || wait.kind !== previous.kind)) {
          // Release the occupied wait so the next real question can mint.
          void notifyHost(previous.kind, previous.sessionId, previous.displayTitle, previous.key, { phase: 'close' })
          lastWait.current = null
        }
        if (!state.prefs.enabled || wait === null || pageIsAttentive()) {
          // Re-arm the NEXT wait unconditionally: a wait that resolves during
          // the completed-title flash must not pin lastKey to its own key, or
          // the same session's next approval would be skipped as a repeat.
          if (wait === null) lastKey.current = null
        ...
        if (state.prefs.title) {
          setTitleAttention(wait.kind, localeName)
          paintFavicon(KIND_META[wait.kind].color)
        }
        if (lastKey.current === wait.key) return
        lastKey.current = wait.key
        // Approvals collapse onto the host waterfall ordinal. Questions /
        // plan-reviews occupy a live slot on the host; this key is only for
        // the RPC contract.
        const key = `${wait.key}:open`
        void (async () => {
          applyOutcome(await notifyHost(wait.kind, wait.sessionId, wait.displayTitle, key), wait.kind, wait)
        })()
      }, [applyOutcome, attention, localeName, notifyHost, state.loaded, state.prefs])
