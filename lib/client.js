...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      intro: 'Get notified when an agent needs approval or finishes a turn while this page is in the background.',
    ...
    const zh = {
      nav: '提醒',
      title: '注意力提醒',
      intro: '当页面不在前台时，agent 需要审批或任务完成会通过标题、声音和系统通知提醒你。',
      master: '启用注意力提醒',
    ...
    function playTone(kind) {
      ...
      const high = kind === 'completed' ? 660 : 880
      const low = kind === 'completed' ? 880 : 660
      first.frequency.value = high
      second.frequency.value = low
      first.type = 'sine'
      second.type = 'sine'
    ...
    function AttentionRoot(props) {
      ...
      React.useEffect(() => {
        const report = () => syncHeartbeat(document.visibilityState)
        report()
      ...
      React.useEffect(() => {
        ...
        const attentive = pageIsAttentive()
        if (attention === null || attentive || !state.prefs.enabled) {
          clearTitleAttention()
          clearFavicon()
        ...
        void (async () => {
          try {
            const result = await rpc(props.connection, 'notify', {
              ...
              title: KIND_META[attention.kind].title[localeName],
              body: attention.displayTitle,
              key: attention.key,
            })
            const usedNative = result.native === true
            store.update({ nativeAvailable: result.nativeAvailable !== false })
            if (!usedNative && state.prefs.browserNotification) {
              showBrowserNotification(attention, localeName, props.sessions)
            }
          } catch {
            if (state.prefs.browserNotification) {
              showBrowserNotification(attention, localeName, props.sessions)
            }
          }
        })()
      }, [attention, localeName, props.connection, props.sessions, state.loaded, state.prefs, store])
    ...
    const toastStyle = {
      ...
      padding: 14,
      borderRadius: 12,
      border: '1px solid var(--dsw-alias-border-light, #e5e7eb)',
    ...
    const secondaryStyle = { margin: 0, color: 'var(--dsw-alias-label-secondary, #6b7280)', fontSize: 13, lineHeight: 1.5 }
