    ...
    function AttentionRoot(props) {
      ...
      React.useEffect(() => {
        ...
        if (!state.prefs.enabled || attention === null || pageIsAttentive()) {
          ...
          // Title cleanup is separate: while the one-shot completed flash owns
          // the title, leave it to the flash timer.
        ...
        // Host-computed session.updatedAt is identical across tabs for the
        // same wait and changes when a new wait arrives (the agent ran a
        // turn in between). Approvals still collapse onto the host waterfall
        // ordinal; questions/plan-reviews use this key as their identity.
        const key = `${attention.key}:${attention.updatedAt ?? 0}`
        void (async () => {
          applyOutcome(await notifyHost(attention.kind, attention.sessionId, attention.displayTitle, key), attention.kind, attention)
        })()
