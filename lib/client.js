    ...
    function AttentionRoot(props) {
      ...
      const applyOutcome = React.useCallback((result, kind, payload) => {
        if (result === null) {
          ...
          if (state.prefs.browserNotification) showBrowserNotification(payload, localeName, props.sessions)
          return
        }
        if (result.sound === true && state.prefs.sound) playTone(kind)
        if (result.fallback === true && state.prefs.browserNotification) {
          showBrowserNotification(payload, localeName, props.sessions)
        }
      }, [localeName, props.sessions, state.prefs])
