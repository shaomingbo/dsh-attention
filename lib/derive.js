 ...
 * @param {readonly { id: string, displayTitle?: string, pendingInteraction?: string, completed?: boolean }[]} rows
 * @param {string | undefined} currentId
 * @param {ReturnType<typeof defaultPrefs>} prefs
 */
export function deriveAttention(rows, currentId, prefs) {
  let best = null
...
export function clientLooksPresent(visibility, heartbeatAt, now, staleMs = HEARTBEAT_STALE_MS) {
  if (typeof heartbeatAt !== 'number') return false
  if (now - heartbeatAt > staleMs) return false
