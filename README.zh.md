...
**单一生产者规则**：浏览器标签活着（心跳 8 秒内新鲜）时，横幅和铃声都归页面，Host 让位；所有标签关闭或冻死时，由 Host 自己出横幅（带内置铃声）——不重复、也不漏。完成和瀑布审批回落到 Host 铸造的会话内序号；提问/计划审阅用所有标签共享的 `updatedAt` 作为身份，恢复中的标签和第二标签无法为同一次等待发明新身份。同一身份的并发上报走 singleflight：Host 只给其中一个调用者授予 `sound` 和 `fallback`。原生发送失败会软记账几秒（回声折叠），之后的重试可以回退。偏好尚未加载完时观察到的完成边沿会先暂存、加载后补发。
