...
**单一生产者规则**：浏览器标签活着（心跳 8 秒内新鲜）时，横幅和铃声都归页面，Host 让位；所有标签关闭或冻死时，由 Host 自己出横幅（带内置铃声）——不重复、也不漏。Host 同时是完成与审批事件的**唯一身份铸造者**：同一边沿的任何上报（备份监听、第一个标签、第二个标签）都归并到同一个会话内序号，并发标签只出一条横幅（且只有被接受的那一方拥有铃声），而真实的新边沿必然拿到新身份。偏好尚未加载完成时观察到的完成边沿会先暂存、加载后补发，启动竞态不会吞掉通知。
